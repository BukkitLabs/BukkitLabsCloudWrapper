...
import net.bukkitlabs.bukkitlabscloud.server.exeption.InvalidJarFileException;
...
public class MinecraftServerFile{
    ...
    public MinecraftServerFile(@NotNull final String minecraftServerFile) throws InvalidJarFileException{
        if (!minecraftServerFile.endsWith(".jar")) {
            throw new InvalidJarFileException("Invalid jar file: " + minecraftServerFile);
        }
        if (!new File(minecraftServerFile).exists() || !new File(minecraftServerFile).isFile()) {
            throw new InvalidJarFileException("File does not exist or is not a regular file: " + minecraftServerFile);
        }
        if (!isMinecraftServerJar(minecraftServerFile)) {
            throw new InvalidJarFileException("Invalid minecraft server jar file: " + minecraftServerFile);
        }
